(function() {
  ...
  __component__$3.options.__file = "/Users/fabian/Sites/stonedonsteel.org/site/plugins/markdown-field/src/components/MarkdownToolbar.vue";
  ...
  __component__$2.options.__file = "/Users/fabian/Sites/stonedonsteel.org/site/plugins/markdown-field/src/components/MarkdownBlock.vue";
  ...
  var _sfc_render$1 = function render() {
    var _vm = this, _c = _vm._self._c;
    return _c("k-field", _vm._b({ staticClass: "k-markdown-field", attrs: { "input": _vm._uid, "counter": _vm.counterOptions }, scopedSlots: _vm._u([{ key: "label", fn: function() {
      return [_c("k-label", { attrs: { "input": _vm._uid, "required": _vm.required, "title": _vm.label, "type": "field" } }, [_c("span", { on: { "click": _vm.focus } }, [_vm._v(" " + _vm._s(_vm.label) + " ")])])];
  ...
  __component__$1.options.__file = "/Users/fabian/Sites/stonedonsteel.org/site/plugins/markdown-field/src/components/MarkdownField.vue";
      ...
      do {
        ...
        if (!inlineDone) {
          ...
          if (isFirstLine) {
            ...
            inlineTokens = inlineTokens.filter(
              (name2) => candidates.includes(name2)
            );
        ...
        enter: ({ node, from: from2, to: to2 }) => {
          if (blockFormats.exists(node.name)) {
            ...
            if (block === "OrderedList") {
              listNumber = parseInt(
                line.text.slice(from2 - line.from, to2 - line.from),
                10
              );
            }
          }
  ...
  function toggleWordFormat(view, blockFormats, inlineFormats, type) {
    ...
    if (activeIndex !== -1) {
      if (activeIndex === 0) {
        ...
        for (const name2 of tokenNames) {
          toggleWordFormat(view, blockFormats, inlineFormats, name2);
        }
        for (const name2 of tokenNames.reverse().slice(1)) {
          toggleWordFormat(view, blockFormats, inlineFormats, name2);
        }
      }
    } else {
  ...
  function renderLine(nodes, blockFormats, inlineFormats) {
    ...
    const progress = (node) => {
      ...
      if (node && marks.some(
        ...
        const [_2, lead, inner2, trail] = /^(\s*)(.*?)(\s*)$/.exec(node.text);
        leading += lead;
        trailing = trail;
        if (lead || trail) {
      ...
      const noEsc = inner && inlineFormats.get(inner).escape === false;
  ...
  function toggleInlineFormatForLine(view, blockFormats, inlineFormats, lineNumber, type, action) {
    ...
    for (const key of blockFormats.types) {
      const match = blockFormats.hasMark(key) && line.text.match(blockFormats.mark(key));
      if (match) {
  ...
  function toggleSelectionInlineFormat(view, blockFormats, inlineFormats, type) {
    ...
    const insert2 = lines.join(view.state.lineBreak);
    const lengthChange = insert2.length - lengthBefore;
    view.dispatch({
      changes: {
        insert: insert2,
        from: firstLine.from,
  ...
  const [nav, doc] = typeof navigator !== "undefined" ? [navigator, document] : [
  ...
  function formatKeyName(keys2, translate, before2 = " (", after2 = ")") {
    const keyName2 = keys2[currentPlatform] ? keys2[currentPlatform] : keys2.key ? keys2.key : null;
    if (keyName2 === null) {
      return "";
    }
  ...
  document.addEventListener(
    "visibilitychange",
    () => document.hidden ? toggleModifierKeydown({ metaKey: false, ctrlKey: false }) : null
  );
  ...
  class Extensions {
    ...
    getPluginsByType(type = "extension") {
      return this.extensions.filter((extension) => extension.type === type).reduce((result2, extension) => [...result2, ...extension.plugins()], []);
    }
    getButtons() {
      return this.extensions.filter((extension) => extension.type === "button").reduce((result2, extension) => [...result2, extension], []);
    }
    getDialogs() {
      return this.extensions.filter((extension) => extension.dialog).reduce((result2, extension) => [...result2, extension], []);
    }
    getFormats(type) {
    ...
    getKeymap() {
      return this.extensions.filter((extension) => extension.keys).reduce((result2, extension) => [...result2, ...extension.keys()], []);
    }
  }
      ...
      parseInline: [
        {
          ...
          parse(cx, next, pos) {
            ...
            const after2 = cx.slice(pos, cx.end);
      ...
      const pasteUrlsPlugin = ViewPlugin.define(() => ({}), {
        eventHandlers: {
          paste(e, view) {
            ...
            const [, prefix, linkText, suffix] = view.state.sliceDoc(from, to).match(/^(\s*)(.*?)(\s*)$/);
            const link = useKirbytext ? `(link: ${pasted} text: ${linkText})` : `[${linkText}](${pasted})`;
            view.dispatch({
              changes: {
                insert: link,
  ...
  function toggleTaskListCheckbox(view, pos) {
    const old = view.state.doc.sliceString(
      pos,
  ...
  class Footnote extends Button {
    ...
    get command() {
      return () => this.editor.dispatch(
        this.editor.state.changeByRange((range2) => ({
          changes: [
            { from: range2.from, insert: "[^" },
            { from: range2.to, insert: "]" }
          ],
  ...
  class Headlines extends Button {
    ...
    get button() {
      return {
        ...
        dropdown: this.dropdownItems().filter(
          (item) => this.options.levels.includes(item.name)
        )
      };
    }
    configure(options) {
      ...
      return [
        {
          name: "h1",
          icon: "h1",
          label: this.input.$t("markdown.toolbar.button.heading.1") + this.formatKeyName(
        ...
        },
        {
          name: "h2",
          icon: "h2",
          label: this.input.$t("markdown.toolbar.button.heading.2") + this.formatKeyName(
        ...
        },
        {
          name: "h3",
          icon: "h3",
          label: this.input.$t("markdown.toolbar.button.heading.3") + this.formatKeyName(
        ...
        },
        {
          name: "h4",
          icon: "h4",
          label: this.input.$t("markdown.toolbar.button.heading.4") + this.formatKeyName(
        ...
        },
        {
          name: "h5",
          icon: "h5",
          label: this.input.$t("markdown.toolbar.button.heading.5") + this.formatKeyName(
        ...
        },
        {
          name: "h6",
          icon: "h6",
          label: this.input.$t("markdown.toolbar.button.heading.6") + this.formatKeyName(
      ...
      this.input.$panel.dialog.open({
        ...
        on: {
          ...
          submit: (values2) => {
            ...
            values2.text = values2.text || contents || null;
    ...
    methods: {
      ...
      createHighlights() {
        ...
        let highlights = this.customHighlights.filter(
          (definition) => this.highlights === true || Array.isArray(this.highlights) && this.highlights.includes(definition.name)
        );
  ...
  __component__.options.__file = "/Users/fabian/Sites/stonedonsteel.org/site/plugins/markdown-field/src/components/MarkdownInput.vue";
  ...
  window.panel.plugin("fabianmichael/markdown-field", {
    ...
    icons: {
      ...
      separator: '<path d="M2 11H4V13H2V11ZM6 11H18V13H6V11ZM20 11H22V13H20V11Z"></path>'
    }
  });
})();

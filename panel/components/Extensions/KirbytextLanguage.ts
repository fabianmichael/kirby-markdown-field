...
import type { FormatDefinition } from '../../types/global';
...
function Kirbytag(knownTags: string[]): MarkdownExtension {
    ...
    parseInline: [
      {
        ...
        parse(cx, next, pos) {
          ...
          const after = cx.slice(pos, cx.end);
          const regex = new RegExp(`(\\((?:${tagNamesPattern}):)|(\\()|(\\))`, 'gi');
...
export default class MarkdownLanguage extends Extension {
  keys(): KeyBinding[] {
    return markdownKeymap.slice() as KeyBinding[];
  }

  plugins(): LanguageSupport[] {

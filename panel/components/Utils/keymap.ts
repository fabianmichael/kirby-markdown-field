...
type Platform = 'mac' | 'win' | 'linux' | 'key';
...
export function formatKeyName(
  ...
  const keyName = keys[currentPlatform] ? keys[currentPlatform] : keys.key ? keys.key : null;
  ...
  if (result === 'Space') {
    result = currentPlatform === 'mac' ? '␣' : (translate('markdown.key.space') as string);
  }

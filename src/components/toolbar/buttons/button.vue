  ...
  methods: {
    ...
    toggleWrap(arg) {
      ...
      if (this.active) {
        ...
        start =
          start.substr(0, startLastIndex) +
          start.substr(startLastIndex + arg.length);
    ...
    toggleInlineFormat(mark) {
      return toggleInlineFormat(this.editor, mark);
    },
    /**
    ...
    toggleCode() {
      ...
      if (this.active) {
        if (
          this.currentTokenType.secondary == "block" ||
    ...
    toggleCodeBlock() {
      ...
      let previousLineToken = this.editor.getTokenAt({
        line: startPoint.line - 1,
        ch: 999999,
      });
      let previousLineIsCode =
        previousLineToken.type !== null
          ? previousLineToken.type.endsWith("blockcode")
          : false;
      ...
      if (this.active) {
        ...
        this.editorDoc.setSelection(
          { line: firstLine, ch: 0 },
          { line: lastLine, ch: 999999 }
        );
        let newRange = this.editorDoc.getRange(
          { line: firstLine + 1, ch: 0 },
          { line: lastLine - 1, ch: 999999 }
        );

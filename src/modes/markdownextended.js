...
// Customized version of the CodeMirror’s Markdown mode, that includes
// tidbits like additional classes used as styling hooks.
// Changed for the "Kirby 3 Markdown Field" plugin:
// Re-distributed under an MIT license (see LICENSE file)
...
CodeMirror.defineMode(
  ...
  function(cmCfg, modeCfg) {
    ...
    var tokenTypes = {
      header: 'header',
      code: 'line-cm-blockcode',
      inlineCode: 'code',
      quote: 'quote',
      list1: 'line-cm-blocklist',
      list2: 'line-cm-blocklist line-cm-blocklist-level-2',
      list3: 'line-cm-blocklist line-cm-blocklist-level-3',
      hr: 'line-cm-blockhr hr',
      image: 'image',
      ...
      if (state.formatting) {
        ...
        for (var i = 0; i < state.formatting.length; i++) {
          ...
          if (!/^(list-|blockcode)/.test(state.formatting[i])) {
            ...
            styles.push('line-cm-' + state.formatting[i]);
          }

          if (state.formatting[i] === 'header') {
            styles.push(
              `${tokenTypes.formatting}-${state.formatting[i]}-${state.header}`,
              `line-cm-blockheader`,
              `line-cm-blockheader-${state.header}`
            );
          }
